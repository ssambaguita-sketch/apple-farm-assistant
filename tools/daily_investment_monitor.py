...
def main():
    ...
    for _, w in watch.iterrows():
        ...
        for f in filings:
            filing_rows.append({"company": company, "ticker": ticker, "corp_code": corp_code, "rcept_no": f.get("rcept_no"), "rcept_dt": f.get("rcept_dt"), "report_nm": f.get("report_nm")})
        row = {"as_of": str(date.today()), "company": company, "ticker": ticker, "corp_code": corp_code, "symbol": symbol, **px, "relevant_filing_count_21d": len(filings), "correction_count": corr, "latest_relevant_filing": filings[0].get("report_nm") if filings else "", "latest_rcept_no": filings[0].get("rcept_no") if filings else ""}
        opp, risk, state, reasons = classify(row)
        row.update({"opportunity_score": round(opp, 1), "risk_score": round(risk, 1), "state": state, "reasons": reasons, "max_position_pct": 0 if state != "WATCH" else (1 if risk >= 50 else 2), "entry_rule": "WATCH는 매수명령이 아님. 최신 상세공시/납입/희석률 확인 후 다음 거래일 2~3회 분할 검토", "stop_rule": "실거래 전환 시 -8% 또는 투자 가정 훼손 시 청산", "time_stop": "20거래일 내 촉매 미발생 시 청산"})
        rows.append(row)
    ...
    summary = {"as_of": str(date.today()), "symbols": len(board), "watch": int((board["state"] == "WATCH").sum()), "research": int((board["state"] == "RESEARCH").sum()), "avoid": int((board["state"] == "AVOID").sum())}
