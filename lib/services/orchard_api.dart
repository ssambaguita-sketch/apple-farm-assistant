...
class OrchardApi {
  Future<List<Map<String, dynamic>>> list() async {
    try {
      final r = await FarmApi.getWithWakeup(Uri.parse('${FarmApi.baseUrl}/api/orchards'));
      if (r?.statusCode == 200) {
  ...
  Future<Map<String, dynamic>> create({
    ...
    try {
      final r = await FarmApi.postWithWakeup(
      ...
      if (r != null) return {'error': _message(r, '과수원 등록 실패')};
      return {'error': '운영 서버가 응답하지 않습니다. 잠시 후 다시 시도하세요.'};
    } catch (e) {
  ...
  Future<Map<String, dynamic>> remove({
    ...
    try {
      ...
      if (r != null) return {'error': _message(r, '과수원 삭제 실패')};
      return {'error': '운영 서버가 응답하지 않습니다. 잠시 후 다시 시도하세요.'};
    } catch (e) {
