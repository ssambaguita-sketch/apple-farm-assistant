...
class FarmApi {
  ...
  static Future<void> initialize() async {
    ...
    _baseUrl = (p.getString(_key) ?? '').trim().replaceAll(RegExp(r'/+$'), '');
  }
  ...
  Future<bool> health() async {
    ...
    try {
      final r = await http.get(Uri.parse('$_baseUrl/health')).timeout(const Duration(seconds: 8));
  ...
  Map<String, dynamic> _demoDashboard(String orchard) => {
    ...
    'best_work_times': [
      {'time': '08:00', 'temp': 22, 'wind': 1.5, 'rain_probability': 10, 'grade': '참고', 'score': 80},
      {'time': '17:00', 'temp': 25, 'wind': 2.0, 'rain_probability': 20, 'grade': '참고', 'score': 75}
    ]
  };
  ...
  Future<List<dynamic>> orchards() async {
    if (_baseUrl.isNotEmpty) {
      try {
        final r = await http.get(Uri.parse('$_baseUrl/api/orchards')).timeout(const Duration(seconds: 8));
        if (r.statusCode == 200) return jsonDecode(r.body);
      } catch (_) {}
    }
    return [{'name': 'A과수원', 'variety': '후지', 'offline_mode': true}];
  }

  Future<Map<String, dynamic>> coach() async {
    if (_baseUrl.isNotEmpty) {
      try {
        final r = await http.get(Uri.parse('$_baseUrl/api/coach')).timeout(const Duration(seconds: 8));
        if (r.statusCode == 200) return Map<String, dynamic>.from(jsonDecode(r.body));
      } catch (_) {}
    }
    return {
  ...
  Future<Map<String, dynamic>> survivorAdvice(Map<String, dynamic> data) async {
    if (_baseUrl.isNotEmpty) {
      try {
        final r = await http.post(Uri.parse('$_baseUrl/api/weeds/survivor-advice'),
          headers: {'Content-Type': 'application/json'}, body: jsonEncode(data)).timeout(const Duration(seconds: 8));
        if (r.statusCode == 200) return Map<String, dynamic>.from(jsonDecode(r.body));
      } catch (_) {}
    }
    return {
      'offline_mode': true,
      'possible_causes': ['잡초 종류 오인 가능성', '처리 시 잡초가 너무 자랐을 가능성', '살포 균일도 문제 가능성', '같은 작용기작 반복 가능성'],
      'actions': [
